...
function createTokenCard(token) {
    ...
    return '<article class="token-card" data-address="' + token.address + '" data-verdict="' + verdict + '" data-score="' + score + '" data-liq="' + (token.liquidity || 0) + '" data-holders="' + (token.top_10_holders_pct || 0) + '" data-age="' + (token.contract_age_hours || 0) + '">' +
        '<div class="card-header">' +
            '<div class="token-info">' +
                ...
                '<div class="token-info-text">' +
                    '<h4 class="token-name" title="' + (token.name || 'Unknown') + '">' + (token.name || 'Unknown') + '</h4>' +
        ...
        '<div class="card-footer">' +
            '<a href="' + birdeyeUrl + '" target="_blank" rel="noopener noreferrer" class="card-link"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15,3 21,3 21,9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>View on Birdeye</a>' +
...
function renderTokens(tokens) {
    ...
    els.grid.innerHTML = tokens.map(createTokenCard).join('');
